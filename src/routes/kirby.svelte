<script>
  ...
  onMount(() => {
    ...
    //gsap.set('#KirbySVG > g > *', {opacity: 0})
    gsap.set('#ring', { opacity: 0 })
    gsap.set('#ringMask', { opacity: 0 })

    function groundTl() {
      const tl = gsap.timeline({ defaults: {} })
      tl.set('#groundLine', { opacity: 1 })
    ...
    function buildingsTl() {
      ...
      tl.addLabel('buildingStart')
        ...
        .from('#towerPole', { scaleY: 0 }, '<')
        .from('#towerMidPole', { scaleY: 0 }, '>')
        .to('#towerBasket', { duration: 0.01, opacity: 1 }, '>')
        .from('#towerBasket', { scaleX: 0, transformOrigin: 'center center' }, '<')
        .from('#towerTopPole', { scaleY: 0 }, '<+0.4')
        .to('#powerPole1', { opacity: 1 }, '>-0.1')
    ...
    function buildingMoss() {
      ...
      mosses.forEach((moss) => {
        ...
        tl.from(moss.children[1].children, { scaleY: 0, stagger: 0.3 }, 'start+=0.35')
      })

      tl.from('#bigBuildingMoss > g > *', { scaleY: 0 }, '<+0.3')
    ...
    baseTl.add(groundTl())
    baseTl.add(buildingsTl())
    baseTl.add(lightsTl(), '-=1.5')
    baseTl.add(buildingMoss(), '-=0.75')
    baseTl.add(treeTrunksTl(), '-=1')
    baseTl.add(treeLeafTl(), '-=1.5')
        ...
        <g id="moss3">
          ...
          <g mask="url(#mask0_8_22)">
            <g id="Group 1">
              <path
                id="mosses0202"
                d="M322 92.5C322 90.567 323.567 89 325.5 89V89C327.433 89 329 90.567 329 92.5V122.5C329 124.433 327.433 126 325.5 126V126C323.567 126 322 124.433 322 122.5V92.5Z"
                fill="#8AB029"
              />
              <path
                id="mosses0101"
                d="M293 100C293 91.7157 299.716 85 308 85H309C317.284 85 324 91.7157 324 100V159C324 167.284 317.284 174 309 174H308C299.716 174 293 167.284 293 159V100Z"
                fill="#8AB029"
              />
              <path
                id="mosses0202_2"
                d="M268 99C268 90.7157 274.716 84 283 84H284C292.284 84 299 90.7157 299 99V145C299 153.284 292.284 160 284 160H283C274.716 160 268 153.284 268 145V99Z"
                fill="#8AB029"
              />
              <path
                id="mosses0303"
                d="M250 94.5C250 88.701 254.701 84 260.5 84V84C266.299 84 271 88.701 271 94.5V123.5C271 129.299 266.299 134 260.5 134V134C254.701 134 250 129.299 250 123.5V94.5Z"
                fill="#8AB029"
              />
              <path
                id="mosses0202_3"
                d="M235 94.9315C235 89.5978 239.324 85.274 244.658 85.274V85.274C249.991 85.274 254.315 89.5978 254.315 94.9315V112.959C254.315 118.293 249.991 122.616 244.658 122.616V122.616C239.324 122.616 235 118.293 235 112.959V94.9315Z"
                fill="#8AB029"
              />
            </g>
          </g>
